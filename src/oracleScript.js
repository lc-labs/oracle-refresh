...
const DEFAULT_TENDERLY_URL = import.meta.env.VITE_TENDERLY_URL ?? ''
...
window.__reserveDevToolsUndefaultCollaterals = async (tenderlyUrl = DEFAULT_TENDERLY_URL) => {
  const provider = new ethers.providers.JsonRpcProvider({ url: tenderlyUrl })
...
const loadOracleFromAddress = async (oracle, provider) => {
...
window.__reserveDevTools = async (tenderlyUrl = DEFAULT_TENDERLY_URL) => {
    ...
    ...(await Promise.all(
      oracles.map(async (oracle) => {
        ...
        const [roundId, price] = await Promise.all([
          aggregatorOracle.callStatic.latestRound({ from: oracle.chainLinkFeed }).catch(() => null),
  ...
  const updateOracleData = async (oracle, newPrice) => {
    ...
    const oracleContract = new ethers.Contract(oracle.chainLinkFeed, OracleAbi, provider)
    ...
    const newEntry = hexConcat([
      ...
      hexZeroPad(parseUnits(newPrice, oracle.oracleDecimals).toHexString(), 24),
    ])
    ...
    oracle.oraclePrice =
      latestAnswer == null ? 0 : parseFloat(formatUnits(latestAnswer, oracle.oracleDecimals))
    ...
    Object.fromEntries(
      await Promise.all(
        plugins.map(async (plug) => {
          const collateralContract = new ethers.Contract(plug.address, CollateralAbi, provider)
          const assetContract = new ethers.Contract(plug.address, AssetAbi, provider)

          const [price, status] = await Promise.all([
            assetContract.callStatic.tryPrice().catch(() => {
    ...
    oracles: Object.fromEntries(
      data.map((oracle) => [
        oracle.name,
        {
          ...
          makePriceRecent: async () => updateOracleData(oracle, oracle.oraclePrice.toString()),
          setPrice: async (newPrice) => updateOracleData(oracle, newPrice),
        },
      ])
    ),
